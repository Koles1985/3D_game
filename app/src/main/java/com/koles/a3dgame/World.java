...
public class World {
    ...
    private void generateInvaders(){
        for(int row = 0; row < 4; row++){
            for(int column = 0; column < 8; column++){
                Invader invader = new Invader(-WORLD_MAX_X / 2 + column * 2f, 0, WORLD_MIN_Z
                + row * 2f);
        ...
        for (int i = 0; i < len; i++){
            ...
            if(invader.state == Invader.INVADER_ALIVE){
                if(random.nextFloat() < 0.001f){
                    Shot shot = new Shot(invader.position.x, invader.position.y, invader.position.z,
                            Shot.SHOT_VELOCITY);
                    shots.add(shot);
                    listener.shot();
                }
            ...
            if(invader.state == Invader.INVADER_DEAD && invader.stateTime >
            Invader.INVADER_EXPLOSION_TIME){
                invaders.remove(i);
                i--;
                len--;
            }
    ...
    private void checkInvaderColisions(){
        ...
        for(int i = 0; i < len; i++){
            Invader invader = invaders.get(i);
            if(OverlapTester.overlapSphere(ship.bounds, invader.bounds)){
                ship.lives = 1;
                ship.kill();
                return;
            }
    ...
    private void checkShotColisions(){
        int len = shots.size();
        ...
        for(int i = 0; i < len; i++){
            ...
            for(int j = 0; j < len2; j++){
                ...
                if(OverlapTester.overlapSphere(shield.bounds, shot.bounds)){
                    shields.remove(j);
                    shots.remove(i);
                    i--;
                    len--;
            ...
            if(shot.velocity.z < 0){
                ...
                for(int j = 0; j < len2; j++){
                    Invader invader = invaders.get(i);
                    if(OverlapTester.overlapSphere(invader.bounds, shot.bounds)
                ...
                if(OverlapTester.overlapSphere(shot.bounds, ship.bounds)
    ...
    public void shoot(){
        ...
        if(System.nanoTime() - lastShotTime > 1000000000 || friendShots == 0){
            shots.add(new Shot(ship.position.x, ship.position.y, ship.position.z,
                    -Shot.SHOT_VELOCITY));
            lastShotTime = System.nanoTime();
            listener.shot();
        }
